...
export default function ComprehensiveHomePage() {
  ...
  // Core Values (static for now, could be dynamic later)
  const coreValues = [
    {
    ...
    const fetchData = async () => {
      try {
        ...
        if (eventsResponse.ok) {
          ...
          const totalParticipants = eventsData.reduce((sum: number, event: Event) => 
            sum + event.currentParticipants, 0
          );
          const totalRaised = eventsData.reduce((sum: number, event: Event) => 
  ...
  const upcomingEvents = events
    ...
    .map(event => ({
      ...
      description: event.description.length > 100 
        ...
        : event.description,
      seats: `${event.currentParticipants}/${event.maxParticipants}`,
  ...
  // Get success stories from events with feedback
  const successStories = events
    .filter(event => event.feedbacks && event.feedbacks.length > 0)
    .slice(0, 4)
    .map((event, index) => {
      ...
      return {
        ...
        role: event.category,
        story: feedback?.comment || event.description.substring(0, 100) + '...',
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white via-gray-50 to-white">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
              {[...Array(4)].map((_, index) => (
                <div key={index} className="animate-pulse">
                  <div className="bg-gradient-to-br from-white to-gray-50 rounded-3xl border border-gray-200/50 p-8 h-64"></div>
                </div>
              ))}
            </div>
        ...
        <div className="container mx-auto px-4 lg:px-8">
          ...
          {loading ? (
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8 mb-12">
              {[...Array(4)].map((_, index) => (
                <div key={index} className="animate-pulse">
                  <div className="bg-white rounded-3xl border border-gray-200/50 p-6 h-80"></div>
                </div>
              ))}
            </div>
                    ...
                    >
                      <div className="bg-white rounded-3xl border border-gray-200/50 p-6 hover:shadow-xl hover:border-blue-200/50 transition-all duration-500 h-full">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <button
                            onClick={() => router.push(`/events/details/${event.id}`)}
      ...
      {/* Partners - You can make this dynamic too */}
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4 lg:px-8">
          ...
          {/* Partners could be fetched from API too */}
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8">
            {[
              { name: "Ministry of Education", logo: "ME", type: "Government" },
              { name: "Ethiopian Universities", logo: "EU", type: "Education" },
              { name: "Local NGOs", logo: "NGO", type: "Non-profit" },
              { name: "Community Leaders", logo: "CL", type: "Community" },
              { name: "Private Sector", logo: "PS", type: "Business" },
              { name: "International Donors", logo: "ID", type: "Funding" },
            ].map((partner, index) => (
              <motion.div
                key={partner.name}
                initial={{
